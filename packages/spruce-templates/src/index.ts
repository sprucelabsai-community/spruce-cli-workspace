...
export const templates = {
    ...
    valueTypes(options: {
        ...
        const fieldTemplatesByType =
            templateItemUtil.groupFieldItemsByNamespace(
                options.fieldTemplateItems
            )

        const template = templateImportUtil.getTemplate(
            'schema/valueTypes.ts.hbs'
        )
    ...
    schemaBuilder(options: SchemaBuilderTemplateItem) {
        ...
        return template({
            ...options,
            builderFunction:
                options.builderFunction ?? DEFAULT_BUILDER_FUNCTION,
        })
    },

    error(options: ErrorOptions) {
        const template = templateImportUtil.getTemplate(
            'error/SpruceError.ts.hbs'
        )
    ...
    async isValidTemplatedDirectory(options: {
        ...
        const filesToCheck =
            await DirectoryTemplateUtility.filesInTemplate(kind)
